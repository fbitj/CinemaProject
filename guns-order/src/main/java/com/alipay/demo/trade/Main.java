...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // 测试查询当面付2.0交易
//                main.test_trade_query("f39a954bfca648fa8d73ce7c3b7bcb61");
...
//        String subject = "meeting影城门店当面付扫码消费";
//
//        // (必填) 订单总金额，单位为元，不能超过1亿元
//        // 如果同时传入了【打折金额】,【不可打折金额】,【订单总金额】三者,则必须满足如下条件:【订单总金额】=【打折金额】+【不可打折金额】
...
//        // 卖家支付宝账号ID，用于支持一个签约账号下支持打款到不同的收款账号，(打款到sellerId对应的支付宝账号)
//        // 如果该字段为空，则默认为与支付宝签约的商户的PID，也就是appid对应的PID
//        String sellerId = "2088102179763701";
//
...
//    }
     ...
     * @param undiscountableAmount 优惠金额
     * @param goodsDetailList 商品详细
     * @return 状态码
     */
    public Map<String,String> test_trade_precreate(String outTradeNo, String totalAmount, String undiscountableAmount, List<GoodsDetail> goodsDetailList) {
...
//                AlipayTradePrecreateResponse response = result.getResponse();
                ...
                // 需要修改为运行机器上的路径
                String outTradeNo1 = response.getOutTradeNo();
                String filePath = String.format("D:\\me_Workspaces\\3rdObject\\alone\\CinemaProject\\guns-order\\src\\main\\resources\\QRcode\\qr-%s.png",outTradeNo1);
                log.info("filePath:" + filePath);
                //上传到老师的阿里云服务器上
                File qrCodeImge = ZxingUtils.getQRCodeImge(response.getQrCode(), 256, filePath);
                FileInputStream fileInputStream = null;
                try {
                    fileInputStream = new FileInputStream(qrCodeImge);
                } catch (FileNotFoundException e) {
                    e.printStackTrace();
                }
                if(aliyunComponent.getOssClient() != null) {
                    OSSClient ossClient = aliyunComponent.getOssClient();
                    ossClient.putObject(aliyunComponent.getOss().getBucket(), "qr-"+outTradeNo1+".png", fileInputStream);
                }
                res.put("code",response.getCode());
                res.put("filePath","qr-"+outTradeNo1+".png");
                break;
